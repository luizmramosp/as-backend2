...
export const getAll = async (filters: GetAllFilters) => { // Função para listar todas as pessoas de um evento X e de um grupo X
    try {
...
type GetOneFilters = { id_event: number; id_group?: number; id?: number; cpf?: string; } // Função para listar apenas uma pessoa de um evento X e de um grupo X
...
type PeopleCreateData = Prisma.Args<typeof prisma.eventPeople, 'create'>['data']; // Função para adicionar pessoas em um evento X e em um grupo X
...
type PeopleUpdateData = Prisma.Args<typeof prisma.eventPeople, 'update'>['data']; // Função para atualzar uma pessoa de um evento X e de um grupo X
...
type DeleteFilters = { id: number; id_event?: number; id_group?: number; } // Função para excluir uma pessoa de um evento X e de um grupo X
